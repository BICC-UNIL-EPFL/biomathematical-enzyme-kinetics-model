...
k_on <- matrix(c(0.5, 0.5, 0.00001, 0.00001), nrow=n, ncol=n, byrow=TRUE)
k_off <- matrix(c(0.4, 0.4, 0.1, 0.1), nrow=n, ncol=n, byrow=TRUE)
k_cat <- matrix(c(0.1, 0.08, 0, 0), nrow=n, ncol=n, byrow=TRUE)
...
res <- tibble(step = rep(0, n_steps), t = rep(0, n_steps), R1 = rep(0, n_steps), R2 = rep(0, n_steps), iter = rep(0, n_steps))
res[1,] <- list(1, 0, 1, 1, 0)
...
while (t_t < t_max) {
  ...
  PI_ <- (1 - (R_tot[1, 1] / q_R1 + R_tot[2, 1] / q_R2) / K_)
  ...
  res[t_l,] <- list(t_l, t_t, R_tot[[1, 1]], R_tot[[2, 1]], steps)
  PI_ <- (1 - (R_tot[1, 1] / q_R1 + R_tot[2, 1] / q_R2) / K_)
}
write_tsv(res, "Model_1_res.tsv.gz")
figure <- ggplot(res) + geom_point(aes(t, R1, color = 'R1')) + geom_point(aes(t, R2, color = 'R2'))
